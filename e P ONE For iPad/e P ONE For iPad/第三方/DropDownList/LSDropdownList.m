...
- (void)drawView {
    ...
    CGFloat height = oldFrame.size.height * self.list.count - 2 * self.borderWidth;
    if (height>=160) {
        height = 160;
    }
    //下拉列表
    self.listView = [[UITableView alloc] initWithFrame:CGRectMake(self.borderWidth, oldFrame.size.height + self.borderWidth + 1, oldFrame.size.width - 2 * self.borderWidth, height)];
